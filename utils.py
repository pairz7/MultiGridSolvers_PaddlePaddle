...
class Utils(object):
    def __init__(self, grid_size=8, device="cpu", bc='dirichlet'):
    ...
    def compute_p2_sparse(self, P_stencil, n, grid_size):
        ...
        if P_stencil.dtype in [paddle.complex64, paddle.complex128]:
            real_part = paddle.gather_nd(paddle.real(P_stencil), paddle.to_tensor(values_indices, dtype='int64'))
            imag_part = paddle.gather_nd(paddle.imag(P_stencil), paddle.to_tensor(values_indices, dtype='int64'))
            shape = [n, grid_size ** 2, (grid_size // 2) ** 2]
            real_scatter = paddle.scatter_nd(paddle.to_tensor(indexes, dtype='int64'), real_part, shape)
            imag_scatter = paddle.scatter_nd(paddle.to_tensor(indexes, dtype='int64'), imag_part, shape)
            P = paddle.complex(real_scatter, imag_scatter)
        else:
            values = paddle.gather_nd(P_stencil, paddle.to_tensor(values_indices, dtype='int64'))
            shape = [n, grid_size ** 2, (grid_size // 2) ** 2]
            values = paddle.cast(values, P_stencil.dtype)
            P = paddle.scatter_nd(paddle.to_tensor(indexes, dtype='int64'), values, shape)
        return P
    ...
    def compute_sparse_matrix(self, stencils, batch_size, grid_size):
        ...
        values = paddle.gather_nd(stencils, paddle.to_tensor(values_indices, dtype='int64'))
        shape = [batch_size, grid_size ** 2, grid_size ** 2]
        values = paddle.cast(values, 'float64')
        tau = paddle.scatter_nd(paddle.to_tensor(indexes, dtype='int64'), values, shape)
        return tau
    ...
    def compute_p2LFA(self, P_stencil, n, grid_size):
        ...
        X, Y = np.meshgrid(np.arange(-1, 2), np.arange(-1, 2))
        P = paddle.zeros([len(theta_y), len(theta_x), batch_size, grid_size ** 2, (grid_size // 2) ** 2], dtype='float64')
        modes = np.array([[np.exp(-1j * (tx * X + ty * Y)) for tx in theta_x] for ty in theta_y])
        fourier_component = paddle.to_tensor(np.tile(modes, (batch_size, 1, 1, 1, 1)), dtype='complex128')
        for ic in range(grid_size // 2):
            ...
            for jc in range(grid_size // 2):
                ...
                for k in range(3):
                    for m in range(3):
                        I = int(K[i, j, k, m])
                        a = fourier_component[:, :, :, k, m] * paddle.reshape(P_stencil[:, ic, jc, k, m], [-1, 1, 1])
                        a = paddle.transpose(a, [1, 2, 0])
                        a_real = paddle.cast(paddle.real(a), 'float64')
                        a_real = paddle.reshape(a_real, [num_modes, num_modes, batch_size, 1, 1])
                        idx = self.idx_array((I, J, int(batch_size), num_modes))
                        updates = paddle.ones([batch_size * (num_modes ** 2)], dtype='float64')
                        shape = [num_modes, num_modes, batch_size, grid_size ** 2, (grid_size // 2) ** 2]
                        P = P + paddle.scatter_nd(paddle.to_tensor(idx, dtype='int64'), updates, shape) * a_real
        return paddle.cast(P, 'complex128')

    def compute_stencil(self, A, grid_size):
        if isinstance(A, paddle.Tensor):
            indices, _ = self.get_indices_compute_A((A.shape[0], grid_size))
            stencil = paddle.reshape(paddle.gather_nd(A, paddle.to_tensor(indices, dtype='int64')), (A.shape[0], grid_size, grid_size, 3, 3))
            return stencil
        else:
            indices = self.get_indices_compute_A_one(grid_size)
            stencil = np.array(A[indices[:, 0], indices[:, 1]]).reshape((grid_size, grid_size, 3, 3))
            return stencil.astype(np.float64)
    ...
    def compute_coarse_matrixLFA(self, model, n, A_stencil, A_matrices, grid_size, bb=True):
        ...
        P_matrix_t = (paddle.transpose(P_matrix)).numpy()
        A_c = P_matrix_t @ A_matrices @ P_matrix
        return A_c, self.compute_stencil(A_c, (grid_size // 2)), P_matrix, P_matrix_t
    ...
    def compute_coarse_matrix_sparse(self, model, A_stencil, A_matrices, grid_size, bb=True):
        if bb == True:
            ...
            P_stencil = model(inputs=A_stencil, black_box=False, phase="Test")
        P_matrix = paddle.to_tensor(self.compute_p2_sparse(P_stencil, P_stencil.shape[0], grid_size), dtype='float64')
        P_matrix_t = paddle.transpose(P_matrix, [0, 2, 1])
        A_matrices = paddle.squeeze(A_matrices)
        temp = P_matrix_t  # 直接用dense张量
        q = paddle.matmul(temp, paddle.to_tensor(A_matrices, dtype='float64'))
        A_c = paddle.transpose(paddle.matmul(temp, paddle.transpose(q, [0, 2, 1])), [0, 2, 1])
        return A_c, self.compute_stencil(paddle.squeeze(A_c), (grid_size // 2)), P_matrix, P_matrix_t
    ...
    def create_coarse_training_set(self, m, pi, num_training_samples, bb=False, epsilon_sparse=False):
        ...
        for i in tqdm(range(additional_num_training_samples // batch_size)):
            ...
            A_stencils_temp = paddle.to_tensor(A_stencils_[i * batch_size:(i + 1) * batch_size], dtype='float64')
            A_matrices__temp = paddle.to_tensor(A_matrices_, dtype='complex128')
    ...
    def mg_levels(self, model, n, A_stencil, A_matrices, grid_size, max_depth=3, bb=False):
        ...
        for i in range(max_depth):
            ...
            A_stencil = paddle.to_tensor([A_stencil])
    ...
    def solve_with_model(self, model, A_matrices, b, initial_guess, max_iterations, max_depth=3, blackbox=False,
        ...
        def prolongation_fn(A, args):
            ...
            paddle_A_stencil = paddle.to_tensor([A_stencil])
            paddle.set_device(self.device)
            if is_blackbox:
                P_stencil = model(inputs=paddle_A_stencil, black_box=True)
            else:
                P_stencil = model(inputs=paddle_A_stencil, black_box=False, phase="Test")
