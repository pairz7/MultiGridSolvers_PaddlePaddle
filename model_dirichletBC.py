...
class Pnetwork(nn.Layer):
    def __init__(self, grid_size=8, device="/cpu:0"):
        ...
        self.device = device
        self.num_layers = 100
        self.linear0 = nn.Linear(9 * 5, 100, bias_attr=False)
        for i in range(1, self.num_layers):
            setattr(self, f"linear{i}", nn.Linear(100, 100, bias_attr=False,
                weight_attr=nn.initializer.TruncatedNormal(std=np.sqrt(2. / 100) * i ** (-1 / 2))))
            setattr(self, f"bias_1{i}", self.create_parameter(shape=[1], default_initializer=nn.initializer.Constant(0.0)))
            setattr(self, f"linear{i+1}", nn.Linear(100, 100, bias_attr=False,
        ...
        self.output_layer = nn.Linear(100, 4, bias_attr=True)
    ...
    def forward(self, inputs, black_box=False, index=None, pos=-1., phase='Training'):
        ...
        right_idx1 = paddle.to_tensor([i for i in range(2, self.grid_size, 2)], dtype='int64')
        right_idx2 = paddle.to_tensor([i for i in range(1, self.grid_size, 2)], dtype='int64')
        left_idx1 = paddle.to_tensor([i for i in range(0, self.grid_size - 1, 2)], dtype='int64')
        left_idx2 = paddle.to_tensor([i for i in range(1, self.grid_size, 2)], dtype='int64')
        up_idx1 = paddle.to_tensor([i for i in range(1, self.grid_size, 2)], dtype='int64')
        up_idx2 = paddle.to_tensor([i for i in range(2, self.grid_size, 2)], dtype='int64')
        down_idx1 = up_idx1
        down_idx2 = left_idx1
        center_idx1 = up_idx1
        center_idx2 = up_idx1
        ...
        if black_box:
            ...
            right_contributions_output = x[:, :-1, :, 0] / (paddle.index_select(x[:, :, :, 1], paddle.to_tensor(jp1, dtype='int64'), axis=1) + x[:, :-1, :, 0])
            left_contributions_output = x[:, 1:, :, 1] / (x[:, 1:, :, 1] + paddle.index_select(x[:, :, :, 0], paddle.to_tensor(jm1, dtype='int64'), axis=1))
            up_contributions_output = x[:, :, :-1, 2] / (x[:, :, :-1, 2] + paddle.index_select(x[:, :, :, 3], paddle.to_tensor(jp1, dtype='int64'), axis=2))
            down_contributions_output = x[:, :, 1:, 3] / (paddle.index_select(x[:, :, :, 2], paddle.to_tensor(jm1, dtype='int64'), axis=2) + x[:, :, 1:, 3])
            # complete right with black box:
            right_contributions_output_bb = paddle.index_select(inputs, right_idx1, axis=1)
            right_contributions_output_bb = paddle.index_select(right_contributions_output_bb, right_idx2, axis=2)
            right_contributions_output_bb = -paddle.sum(right_contributions_output_bb[:, :, :, 0, :], axis=-1) / paddle.sum(right_contributions_output_bb[:, :, :, 1, :], axis=-1)
            right_contributions_output_bb = paddle.reshape(right_contributions_output_bb[:, -1, :], (1, 1, -1))
            right_contributions_output = paddle.concat([right_contributions_output, right_contributions_output_bb], axis=1)
            left_contributions_output_bb = paddle.index_select(inputs, left_idx1, axis=1)
            left_contributions_output_bb = paddle.index_select(left_contributions_output_bb, left_idx2, axis=2)
            left_contributions_output_bb = -paddle.sum(left_contributions_output_bb[:, :, :, 2, :], axis=-1) / paddle.sum(left_contributions_output_bb[:, :, :, 1, :], axis=-1)
            left_contributions_output_bb = paddle.reshape(left_contributions_output_bb[:, 0, :], (1, 1, -1))
            left_contributions_output = paddle.concat([left_contributions_output_bb, left_contributions_output], axis=1)
            up_contributions_output_bb = paddle.index_select(inputs, up_idx1, axis=1)
            up_contributions_output_bb = paddle.index_select(up_contributions_output_bb, up_idx2, axis=2)
            up_contributions_output_bb = -paddle.sum(up_contributions_output_bb[:, :, :, :, 0], axis=-1) / paddle.sum(up_contributions_output_bb[:, :, :, :, 1], axis=-1)
            up_contributions_output_bb = paddle.reshape(up_contributions_output_bb[:, :, -1], (1, -1, 1))
            up_contributions_output = paddle.concat([up_contributions_output, up_contributions_output_bb], axis=-1)
            down_contributions_output_bb = paddle.index_select(inputs, down_idx1, axis=1)
            down_contributions_output_bb = paddle.index_select(down_contributions_output_bb, down_idx2, axis=2)
            down_contributions_output_bb = -paddle.sum(down_contributions_output_bb[:, :, :, :, 2], axis=-1) / paddle.sum(down_contributions_output_bb[:, :, :, :, 1], axis=-1)
            down_contributions_output_bb = paddle.reshape(down_contributions_output_bb[:, :, 0], (1, -1, 1))
            down_contributions_output = paddle.concat([down_contributions_output_bb, down_contributions_output], axis=-1)
        ...
        up_right_contribution = paddle.index_select(up_right_contribution, right_idx1, axis=2)
        up_right_contribution = up_right_contribution[:, :, :, 0, 1]
        right_up_contirbution = paddle.index_select(inputs, right_idx1, axis=1)
        right_up_contirbution = paddle.index_select(right_up_contirbution, right_idx1, axis=2)
        ...
        ru_center_ = paddle.index_select(ru_center_, right_idx1, axis=2)
        ...
        up_left_contribution = paddle.index_select(up_left_contribution, right_idx1, axis=2)
        up_left_contribution = up_left_contribution[:, :, :, 2, 1]
        left_up_contirbution = paddle.index_select(inputs, left_idx1, axis=1)
        left_up_contirbution = paddle.index_select(left_up_contirbution, right_idx1, axis=2)
        ...
        lu_center_ = paddle.index_select(lu_center_, right_idx1, axis=2)
        ...
        down_left_contribution = paddle.index_select(down_left_contribution, left_idx1, axis=2)
        down_left_contribution = down_left_contribution[:, :, :, 2, 1]
        left_down_contirbution = paddle.index_select(inputs, left_idx1, axis=1)
        left_down_contirbution = paddle.index_select(left_down_contirbution, left_idx1, axis=2)
        ...
        ld_center_ = paddle.index_select(ld_center_, left_idx1, axis=2)
        ...
        down_right_contribution = paddle.index_select(down_right_contribution, left_idx1, axis=2)
        down_right_contribution = down_right_contribution[:, :, :, 0, 1]
        right_down_contirbution = paddle.index_select(inputs, right_idx1, axis=1)
        right_down_contirbution = paddle.index_select(right_down_contirbution, left_idx1, axis=2)
        ...
        rd_center_ = paddle.index_select(rd_center_, left_idx1, axis=2)
